...
export async function renderSleepCharts(
  ...
  const [durationBars, stageStackedBars, efficiencyLine] = await Promise.all([
    renderDurationBars(data),
    renderStageStackedBars(data),
    renderEfficiencyLine(data),
  ]);
...
async function renderDurationBars(data: SleepData) {
  ...
  const config: ChartConfiguration = {
    ...
    options: {
      plugins: {
        title: { display: true, text: 'Sleep Duration', font: { size: 16 } },
...
async function renderStageStackedBars(data: SleepData) {
  ...
  const config: ChartConfiguration = {
    ...
    options: {
      plugins: {
        title: { display: true, text: 'Sleep Stages', font: { size: 16 } },
...
async function renderEfficiencyLine(data: SleepData) {
  ...
  const config: ChartConfiguration = {
    ...
    options: {
      plugins: {
        title: { display: true, text: 'Sleep Efficiency', font: { size: 16 } },
        legend: { display: false },
      },
      scales: {
        x: { ticks: { maxRotation: 45, autoSkip: true, maxTicksLimit: 10 } },
